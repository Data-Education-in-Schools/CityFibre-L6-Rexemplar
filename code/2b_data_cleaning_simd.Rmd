...
**outputdataset** <- **inputdataset** %>%
  select(**list of vars separated by commas**) %>%
  st_drop_geometry()

#The final function will drop the geometry column that is not required
```

## Convert string columns to numerics

Use `head()` to review the first few rows of the output dataset
```{r}
# Quick look at output dataset
```
Use `summary()` to review the format of the output dataset
```{r}
# Summary of output dataset
```
...
```{r}
**cleanoutput** <- mutate(**inputdataset**, 
                     **var1** = as.numeric(str_remove(**var1**, "%")),
                     **var2** = as.numeric(str_remove(**var2**, "%")),
                     **var3** = as.numeric(str_remove(**var3**, "%")))
```
