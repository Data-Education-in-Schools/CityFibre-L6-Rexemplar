...
Draw the boundaries using the leaflet package
```{r}

map <- leaflet(cf_focus_latlong_sf) %>%
...
# Compare with the second scoring approach

Merge in data to 
```{r}
cf_focus_latlong2 <- cf_pc_focus2 %>%
  select(postcode2, scheme_references, ur_class, simd10, simd_rank, nobroadband_rate) %>%
  left_join(pc_latlong, by = "postcode2") %>%
  filter(!is.na(geometry))
```

Add in the pop-up labels
```{r}
cf_focus_latlong2 <- cf_focus_latlong2 %>%
  mutate(pop_label = paste(sep = "<br/>",
                           paste0("<b>",postcode,"</b>"),
                           paste0("Scheme: ", scheme_references), 
                           paste0("Urban Rural Classification: ", ur_class), 
...
Convert to a spatial file type
```{r}
cf_focus_latlong_sf2 <- st_as_sf(cf_focus_latlong2)
```

Draw the second map using different colours
```{r}
map2 <- leaflet(cf_focus_latlong_sf2) %>%
  setView(lng = centre_long, lat = centre_lat, zoom = 7) %>%
  addTiles() %>%
  addPolygons(weight = 2, 
             color = "#5e31ad", 
...
```{r}
saveWidget(map2, file = file.path(output_file_path, "cf_prioritypc2.html"))
```

Add both maps together for comparison
```{r}
map3 <- leaflet() %>%
  setView(lng = centre_long, lat = centre_lat, zoom = 7) %>%
  addTiles() %>%
  ...
  addPolygons(data = cf_focus_latlong_sf2,
              weight = 2, 
             color = "#5e31ad", 
             stroke = TRUE, 
             fillOpacity = 0.5,
...
Write out the third map
```{r}
saveWidget(map3, file = file.path(output_file_path, "cf_prioritypc3.html"))
```


It can clearly be seen that the choice of scoring method has a very large impact on the prioritised postcodes.